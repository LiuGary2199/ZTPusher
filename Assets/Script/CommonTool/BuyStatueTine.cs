...
{
    public static void Crude<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class BuyStatueTine : WhigSuccessor<BuyStatueTine>
{
    // Start is called before the first frame update
    public bool EarnPeople;
    public bool GoFolk;

    static public BuyStatueTine instance;
    ...
    /// <returns></returns>
    public Vector3 LeoSixthSteep(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject BuyFertileDramFreeze(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float SteepAnIrish(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Irish180An360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int BuyRoundBroadleaf(Vector2 p1, Vector2 p2)
    {
        float angle = Irish180An360(SteepAnIrish(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 LeoAngularGistGift(Renderer Renderer)
    {
    ...
    public Vector2 LeoAngularGistGift(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 LeoTranceGift(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 LeoGroupDistrict()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = LeoTargetStark();
        float screenH = LeoTargetRename();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int LeoMirageImage(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] BuyDoctorSequence(int total, int count)
    {
    ...
    public Dictionary<string, object> LaunchAnRespective(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string RespectiveAnLaunch(Dictionary<string, object> Info)
    {
    ...
    public int BitCoalDiff(string dateStart, DateTime dateEnd)
    {
    ...
    public int WetCoalWold(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan CoalWold(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 BuyTugBeAfar(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = BuyTugBeAfar(canvas, text, strFragmentIndex);
            Vector3 lastPos = BuyTugBeAfar(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = BuyTugBeAfar(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 BuyTugBeAfar(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float ExchangeByLure(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Exam= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static  string PremiumDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Exam;
