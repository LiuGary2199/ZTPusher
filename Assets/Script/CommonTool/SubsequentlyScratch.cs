...
/// 负责处理跨平台的通知功能
/// </summary>
public class SubsequentlyScratch : MonoBehaviour
{
    public static SubsequentlyScratch Ductless{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string EyeOffend= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(NeutralSubsequentlySpeculator());
    }

    IEnumerator NeutralSubsequentlySpeculator()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void ComparisonIndiscernible()
    {
        AssetSubsequently();
        WeldonSubsequentlyAnxiety();
    }

    public void AssetSubsequently()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void WeldonSubsequentlyAnxiety()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void SendImaginarySubsequently(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void IntruderSubsequently(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(EyeOffend))
            {
                Application.OpenURL(EyeOffend);
                Debug.Log($"通过URL scheme({EyeOffend})唤起游戏");
            }
        }
    }
    #endif
