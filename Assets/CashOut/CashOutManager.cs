...
public class CashOutManager : WhigSuccessor<CashOutManager>
{
    [Header("短剧后台的产品id")]
    ...
    private void Start()
    {
        Account = AutoTineScratch.BuyLaunch("CashOut_Account");
        Money = AutoTineScratch.BuyIndus("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {BisHeadCar.instance.BuckleTine.CashOut_MoneyName} have been converted,Please check your rewaeds!";
            SubsequentlyScratch.Ductless.AssetSubsequently();
            SubsequentlyScratch.Ductless.IntruderSubsequently(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                SubsequentlyScratch.Ductless.IntruderSubsequently(title, info, (int)Seconds + (i * 10800));
        }
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", AutoTineScratch.BuyLaunch("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        if (Application.platform == RuntimePlatform.IPhonePlayer)
            Platform = "iOS";
        string DeviceAdId = AutoTineScratch.BuyLaunch("gaid");
        if (Application.platform == RuntimePlatform.IPhonePlayer)
            DeviceAdId = AutoTineScratch.BuyLaunch("idfv");
        ...
        BisSlotScratch.BuyDuctless().GibeCardOnto(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                ...
                try
                {
                    ...
                    {
                        //刷新token 获取提现规则
                        AutoTineScratch.YouLaunch("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(AutoTineScratch.BuyLaunch("CashOut_ConvertTime"));
                        if (Data.ConvertTime < ConvertTime.Ticks)
                        {
                            Money = 0;
                            AutoTineScratch.YouIndus("CashOut_Money", Money);
                        }
                        Data.ConvertTime = ConvertTime.Ticks;
                        AutoTineScratch.YouLaunch("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    else
                    {
                        ...
                        ThoseScratch.BuyDuctless().BuryThose("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                ThoseScratch.BuyDuctless().BuryThose("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        BisSlotScratch.BuyDuctless().HttpBuy(
            url: url,
            success: (result) =>
            {
                ...
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = AutoTineScratch.BuyExpend("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                CardHonorDecode.BuyDuctless().SaltHonor("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        AutoTineScratch.YouExpend("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            ThoseScratch.BuyDuctless().BuryThose($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        BisSlotScratch.BuyDuctless().GibeCardOnto(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    ThoseScratch.BuyDuctless().BuryThose("Withdraw fail :" + e.Message);
    ...
    {
        CardHonorDecode.BuyDuctless().SaltHonor("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = ElicitNoseScratch.Instance.BuyElicitFend();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        CardHonorDecode.BuyDuctless().SaltHonor("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
