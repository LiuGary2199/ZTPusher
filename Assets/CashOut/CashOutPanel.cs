...
public class CashOutPanel : ForkUIVisit
{
    ...
    private void Start()
    {
        CashOutManager.BuyDuctless()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { ShaftUIWish(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = BisHeadCar.instance.BuckleTine.CashOut_MoneyName;
        InfoText.text = BisHeadCar.instance.BuckleTine.CashOut_Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = BisHeadCar.instance.ForkBay;
        ContactBtn.onClick.AddListener(() => { Application.OpenURL(BisHeadCar.instance.ForkBay); });
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            OpenPanel(RecordPanel);
            StartLoadingAnim(RecordPanel_Loading);
            CashOutManager.BuyDuctless().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.BuyDuctless().Account;
        Confirm_AccountText.text = CashOutManager.BuyDuctless().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.BuyDuctless().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = BisHeadCar.instance.ForkBay + "/privacy_policy.html";
        ...
        PolicyToggle.isOn = AutoTineScratch.BuyGirl("CashOut_PolicyAgree");
    ...
    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        ...
        CashOutManager.BuyDuctless().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.BuyDuctless().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.BuyDuctless().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            ThoseScratch.BuyDuctless().BuryThose("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            ThoseScratch.BuyDuctless().BuryThose(AccountErrorText.text);
            return;
        }

        CashOutManager.BuyDuctless().Account = AccountInput.text;
        AutoTineScratch.YouLaunch("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            ThoseScratch.BuyDuctless().BuryThose("Please agree to the policy");
            return;
        }
        AutoTineScratch.YouGirl("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.BuyDuctless().Withdraw();
    }
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.BuyDuctless().Data.Record == null || CashOutManager.BuyDuctless().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.BuyDuctless().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.BuyDuctless().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            ThoseScratch.BuyDuctless().BuryThose("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        CardHonorDecode.BuyDuctless().SaltHonor("1301", LeftTimeText.text, CashOutManager.BuyDuctless().Money.ToString());
    }
}
