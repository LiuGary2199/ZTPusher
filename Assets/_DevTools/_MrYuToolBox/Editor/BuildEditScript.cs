...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkBay != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
...
#endif
        if (Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("ADScratch key");
        }
        if (Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].DramBomb != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADScratch>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<ElicitNoseScratch>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<ElicitNoseScratch>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<ElicitNoseScratch>();
            }
        }
        GameObject.Find("MainManager").GetComponent<ElicitNoseScratch>().StrongID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<MailMyScratch>())
        {
            GameObject.Find("MainManager").AddComponent<MailMyScratch>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<MailMyScratch>().Range = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<MailMyScratch>().appid = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].DramBomb = info.GameCode;
        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkBay = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkFacetBay = info.BaseUrl + CBuckle.FacetBay;
        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkBuckleBay = info.BaseUrl + CBuckle.BuckleBay;
        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkUserBay = info.BaseUrl + CBuckle.UserBay;
        Resources.FindObjectsOfTypeAll<BisHeadCar>()[0].ForkElicitBay = info.BaseUrl + CBuckle.ElicitBay;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CBuckle.BuckleBay + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CBuckle.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CBuckle.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) => {
           ...
           //{
               ...
               if (GameObject.Find("BisSlotScratch"))
               {
                    DestroyImmediate(GameObject.Find("BisSlotScratch"));
               }
           //}
       },
       () => {
