...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, GlassFlier> UIAudioGroup;
    Dictionary<string, GlassFlier> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, GlassFlier>();
        SceneAudioGroup = new Dictionary<string, GlassFlier>();
        ...
        Dictionary<string, GlassFlier> audioDic = new Dictionary<string, GlassFlier>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(TheirRear.UIMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            GlassFlier model = new GlassFlier();
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(TheirRear.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            GlassFlier model = new GlassFlier();
    ...
    public Dictionary<string, GlassFlier> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, GlassFlier> audioGroup = JsonMapper.ToObject<Dictionary<string, GlassFlier>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, GlassFlier> dic = new Dictionary<string, GlassFlier>();
        MergeDictionaries.Crude(dic, UIAudioGroup);
        MergeDictionaries.Crude(dic, SceneAudioGroup);
